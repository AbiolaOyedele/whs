/**
 * Method:   POST   create a saved item
 *           PUT    replace one (?id=)
 *           DELETE remove one (?id=)
 * Path:     /api/v1/admin/templates/items
 * Auth:     admin session cookie
 * Response: 200 { item } for writes, 200 { ok: true } for a delete
 *           | { error: { code, message } }
 */
...
import { savedItemSchema } from '@/lib/schemas/templates'
import { deleteTemplateEntry, saveItem } from '@/lib/admin/templates'
...
export const POST: APIRoute = async (context) => {
  try {
    ...
    const input = parseTemplateInput(savedItemSchema, await readBody(context.request))
    return json({ item: await saveItem(null, input) })
  } catch (error) {
...
export const PUT: APIRoute = async (context) => {
  try {
    ...
    const input = parseTemplateInput(savedItemSchema, await readBody(context.request))
    return json({ item: await saveItem(id, input) })
  } catch (error) {
    return toErrorResponse(error)
  }
}
...
export const DELETE: APIRoute = async (context) => {
  try {
    await authorise(context)
    await deleteTemplateEntry('item', requireId(context.url))
    return json({ ok: true })
  } catch (error) {
    return toErrorResponse(error)
  }
